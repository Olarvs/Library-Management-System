...
namespace LibrarySystem
{
    public partial class frmBooks : Form
    {
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            ...
            funct.clearTxt(this);
            sql = "SELECT `AccessionNo`, `BookTitle`, `BookDesc` as 'Description'," +
                "`Author`, `PublishDate`, `BookPublisher`, `Category`, BookType as 'TypeOfBooks', `BookPrice` as" +
                " 'Price',DeweyDecimal,Remark,Status FROM `tblbooks` b, `tblcategory` c WHERE b.`CategoryId`=c.`CategoryId` ";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Accession number is already in used.", "InValid", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            } 
            ...
            sql = "Insert Into `tblbooks` " +
                    ...
                    " VALUES ('" + txtAccessionNo.Text + "','" + txtTitle.Text + "','" + txtDesc.Text + "','" + txtAuthor.Text + "','" + datePublish + "','" + txtPublisher.Text + "','" + cboCategory.SelectedValue + "','0',1, 'Available' ,'" + "',1,'" + remarks + "','" + cboType.Text + "')";
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            sql = "Update `tblbooks` Set `BookTitle`='" + txtTitle.Text +
                    ...
                    "', `BookPublisher`='" + txtPublisher.Text + "', `CategoryId`='" + cboCategory.SelectedValue + "', `DeweyDecimal`='" + 
        ...
        {
            sql = "UPDATE `tblbooks` Set `Status` = 'Unavailable' WHERE `AccessionNo` = '" + txtAccessionNo.Text + "'"; 
            config.Execute_CUD(sql, "failed to execute the query.", "Books has been deleted in the database"); 

            sql = "UPDATE `tblbooknumber` Set `Status` = 'Unavailable 'WHERE `BOOKTITLE`='" + txtTitle.Text + "'";
            config.Execute_Query(sql);
        ...
        {
            sql = "UPDATE `tblbooks` Set `Status` = 'Available' WHERE `AccessionNo` = '" + txtAccessionNo.Text + "'";
            config.Execute_CUD(sql, "failed to execute the query.", "Books has been deleted in the database");

            sql = "UPDATE `tblbooknumber` Set `Status` = 'Available 'WHERE `BOOKTITLE`='" + txtTitle.Text + "'";
            config.Execute_Query(sql);
